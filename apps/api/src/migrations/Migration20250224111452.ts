...
export class Migration20250224111452 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`round\` (\`id\` integer not null primary key autoincrement, \`player_one_choice\` text not null, \`player_two_choice\` text not null, \`created_at\` datetime not null, \`updated_at\` datetime not null, \`game_id\` integer not null, constraint \`round_game_id_foreign\` foreign key(\`game_id\`) references \`game\`(\`id\`) on update cascade);`)
